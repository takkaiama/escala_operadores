...
let state = { categoria:'MOTORISTA', funcao:'', data:null, search:'', changedOnly:false, selectedEmployee:null, auth:null, authToken:localStorage.getItem('escala_auth_token')||'' };
...
function updateAuthUI(){
  ...
  document.body.classList.toggle('can-edit',logged);
  ...
  el('btnNovo').classList.toggle('hidden',!logged);
  el('btnFerias').classList.toggle('hidden',!logged);
...
function logout(){clearAuth();updateAuthUI();render();toast('Sessão encerrada. Modo somente visualização.')}
...
function renderTable(){
  ...
  if(canEdit())tabela.querySelectorAll('[data-action="cell"]').forEach(td=>td.addEventListener('dblclick',()=>openAdjustment(Number(td.dataset.id),Number(td.dataset.day))));
}
